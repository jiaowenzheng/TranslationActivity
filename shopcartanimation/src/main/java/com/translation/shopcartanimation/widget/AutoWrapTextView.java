package com.translation.shopcartanimation.widget;
...
public class AutoWrapTextView extends View {
    ...
    private void initStyle(Context context, AttributeSet attrs) {
        ...
        mEndText = typedArray.getString(R.styleable.AutoWrapTextViewStyle_endText);
        mLineCount = typedArray.getInt(R.styleable.AutoWrapTextViewStyle_lineCount,0);
    ...
    private void splitText(int heightMode) {
        ...
        if (mSplitTextList != null && mSplitTextList.size() > 0){
            int lineSize = mSplitTextList.size();
            if (mLineCount > 0 && lineSize > mLineCount){
                int deleteLine = lineSize - mLineCount;
                for (int i = 0;i < deleteLine ;i++){
                    mSplitTextList.remove(mLineCount);
                }
            }
        }

        Log.i("jiao","splitText size="+mSplitTextList.size());
    ...
    public void drawText(Canvas canvas) {
        ...
        Log.i("jiao","drawText size="+mSplitTextList.size());
        ...
        for (int m = 0, length = mSplitTextList.size(); m < length; m++) {
            ...
            if (!TextUtils.isEmpty(mEndText) && m == length - 1){
                lineText = lineText.substring(0,lineText.length() - mEndText.length()) + mEndText;
            }
